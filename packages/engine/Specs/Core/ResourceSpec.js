...
const createTestSuite = (useFetch) =>
  function () {
    const dataUri =
    ...
    if (useFetch) {
      it("fetchJson calls fetch with expected parameters and parses result", function () {
        ...
        const fetchSpy = spyOn(Resource.prototype, "fetch").and.callThrough();
    ...
    it("head calls correct method", function () {
      ...
      if (useFetch) {
        const fakeResponse = {
          status: 200,
          headers: new Headers(expectedResult),
        };
    ...
    describe("fetchImage without ImageBitmap", function () {
      ...
      it("Calls load with blob response type if headers is set", function () {
      ...
      it("Doesn't call load with blob response type if headers is set but is a data URI", function () {
        spyOn(Resource._Implementations, "load").and.callFake(
          function (
            url,
            responseType,
    ...
    describe("load", function () {
      ...
      const requestBackend = useFetch ? "fetch" : "XHR";
      describe(`URL loading using ${requestBackend}`, function () {
      ...
      describe(`URL loading using mocked ${requestBackend}`, function () {
        let fakeRequest;
        let fakeRequestValue;
          ...
          beforeEach(function () {
            fakeRequest = {
              simulateError: function () {
                fakeRequestValue.reject();
              },
              simulateHttpResponse: function (statusCode, response) {
                fakeRequestValue.resolve({
                  ok: statusCode >= 200 && statusCode < 300,
                  status: statusCode,
                  text: () => Promise.resolve(response),
                });
              },
              simulateResponseXMLLoad: function (responseXML) {
                fakeRequestValue.resolve({
                  ok: true,
                  status: 200,
              ...
              simulateResponseTextLoad: function (responseText) {
                fakeRequest.simulateHttpResponse(200, responseText);
              },
            };

            requestConstructorSpy = spyOn(window, "fetch").and.callFake(
              function () {
                fakeRequestValue = defer();
                return fakeRequestValue.promise;
              },
            );
          });
        } else {
          beforeEach(function () {
            fakeRequestValue = jasmine.createSpyObj("XMLHttpRequest", [
              "send",
              "open",
              "setRequestHeader",
              "abort",
            ...
            fakeRequest = {
              simulateError: function () {
                fakeRequestValue.response = "";
                if (typeof fakeRequestValue.onerror === "function") {
                  fakeRequestValue.onerror();
                }
              },
              simulateHttpResponse: function (statusCode, response) {
                fakeRequestValue.status = statusCode;
                fakeRequestValue.response = response;
                if (typeof fakeRequestValue.onload === "function") {
                  fakeRequestValue.onload();
                }
              },
              simulateResponseXMLLoad: function (responseXML) {
                fakeRequestValue.status = 200;
                fakeRequestValue.responseXML = responseXML;
                if (typeof fakeRequestValue.onload === "function") {
                  fakeRequestValue.onload();
                }
              },
            ...
            requestConstructorSpy = spyOn(
              window,
              "XMLHttpRequest",
            ).and.returnValue(fakeRequestValue);
          });
        }
        ...
        describe("returns a promise that rejects when the request", function () {
          ...
          it("results in an HTTP status code less than 200", function () {
            ...
            fakeRequest.simulateHttpResponse(199);
          ...
          it("is an image with status code 204 with preferImageBitmap", function () {
            ...
            fakeRequest.simulateHttpResponse(204);
        ...
        describe("returns a promise that resolves when the request loads", function () {
          it("a null response with a '' responseType and non-null responseXML with child nodes", function () {
            ...
            fakeRequest.simulateResponseXMLLoad(responseXML);
          ...
          it("a null response with a '' responseType and non-null responseText", function () {
            ...
            fakeRequest.simulateResponseTextLoad(responseText);
        ...
        describe("retries when Resource has the callback set", function () {
          it("rejects after too many retries", function () {
            ...
            const promise = load(resource);

            expect(promise).toBeDefined();

            fakeRequest.simulateError(); // This should retry
            // Wait for the next request to be created, then simulate error
            pollToPromise(function () {
              return requestConstructorSpy.calls.count() > 1;
            }).then(function () {
              fakeRequest.simulateError(); // This fails because we only retry once
            });
          ...
          it("rejects after callback returns false", function () {
            ...
            fakeRequest.simulateError(); // This fails because the callback returns false
            return promise
              .then(function () {
                fail();
              })
          ...
          it("resolves after retry", function () {
            ...
            pollToPromise(function () {
              ...
              fakeRequest.simulateHttpResponse(200, "OK");
            });
...
describe("Core/Resource - XHR", createTestSuite(false));
describe("Core/Resource - Fetch", createTestSuite(true));
